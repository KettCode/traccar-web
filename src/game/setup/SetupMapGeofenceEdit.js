...
const SetupMapGeofenceEdit = ({ selectedGeofenceId }) => {
  const theme = useTheme();
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const t = useTranslation();
  const returnGameId = Number(searchParams.get('returnGameId')) || 0;
  ...
  useEffect(() => {
    const listener = async (event) => {
      ...
      try {
        ...
        if (returnGameId) {
          await fetchOrThrow(`/api/setup/games/${returnGameId}/geofences`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify([
              { geofenceId: item.id, name: item.name, type: 'playfield', role: null },
            ]),
          });
          navigate(`/game/setup/${returnGameId}/wizard?step=geofences`);
        } else {
  ...
  }, [dispatch, navigate, draw, t, returnGameId]);
...
export default SetupMapGeofenceEdit;
